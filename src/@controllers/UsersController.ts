  ...
  async create(req: Request, res: Response): Promise<void> {
    try {
      ...
      const data: UserModelInterface = {
        ...
        password: generateMD5(req.body.password + process.env.SECRET_KEY || '123'),
      ...
      UserModel.create<UserModelInterface>(data, (err, user) => {
        ...
        const url = `${backendUrl}/auth/verify?hash=${data.confirmHash}`
        ...
        sendEmail(
          {
            ...
            html: `Click the <a href=${url}>link</a> to confirm your email.`,
          },
          (err: Error | null) => {
            if (err) {
              res.status(500).json({
